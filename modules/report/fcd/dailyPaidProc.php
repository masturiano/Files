...
//$userAcess = $dailyPaidProcObj->login($userName,$passWord);
//$userNotExist = $dailyPaidProcObj->getRecCount($dailyPaidProcObj->loginNotExist($userName,$passWord));
...
switch($_GET['action']){
	...
	case 'Print':
		...
		echo "window.open('dailyPaidProc_xls.php?{$_SERVER['QUERY_STRING']}');";	
		//$dailyPaidProcObj->dailyPaidProcessCsv($GET['txtDateFrom'],$GET['txtDateTo'],$GET['cmbOrgId']);
...
$arrOrgId = array('0'=>"All",'85'=>"PPCI",'87'=>"JR",'133'=>"PUREGOLD SUBIC",'89'=>"DUTY FREE CLARK",'91'=>"DUTY FREE SUBIC",'153'=>"DAILY COMMODITIES INC",'113'=>"FIRST LANE SUPERTRADERS CO INC");
?>
	...
	<head>
		...
		function printCsv(){
			...
			if (valDateStartEnd(dateStart,dateEnd,'txtDateFrom','txtDateTo')){
				printDailyPaid(dateStart,dateEnd,orgId);
			}	
		}
		
		function printDailyPaid(dateStart,dateEnd,orgId) {
		
				$.ajax({
					url: 'dailyPaidProc.php',
					type: "GET",
		...
		$(function() {
			$( "#tags" ).autocomplete({
				source: function(request, response) {
					$.getJSON('dailyPaidProc.php?action=GETAUTO', {
						term: request.term
					}, response);
				},
            ...
            <form name="formInq" id="formInq">
                <div class="dvContainer">
                	<table border=0 align="center">
						<th colspan="6">
                        	<h4 align="center"><font style="font-family:Lucida Handwriting"> Daily Paid Process</font></h4>
                        </th>
						<tr>
							<td colspan = "6"> </td>
						</tr>
						...
						<tr>
							...
							<td><font style="font-size:13px"> : </font></td>
							<td><? $dailyPaidProcObj->DropDownMenu($arrOrgId,'cmbOrgId','','class="selectBox ui-widget-content ui-corner-all"'); ?></td>
						</tr>
						<tr>
							<td colspan = "6"> </td>
						</tr>
