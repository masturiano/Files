...
class progressBillObj extends commonObj {
    ...
    # IMPORT INVOICE DETAIL  TO SUMMARY
    
    function importInvoiceDetail(){
        $sql = "  
            INSERT INTO 
                ORS_tblProgressSummary
                    (INVOICE_ID,SEGMENT1,VENDOR_NAME,ORG_ID,VENDOR_SITE_CODE,
                    INVOICE_NUM, INVOICE_DATE, INVOICE_AMOUNT, SOURCE, DESCRIPTION,AMOUNT_REMAINING)  
            SELECT 
                INVOICE_ID, SEGMENT1, VENDOR_NAME, ORG_ID, VENDOR_SITE_CODE, 
    ...
    # IMPORT PAY TO SUMMARY
    
    function importPayDetail(){
        $sql = "    
            INSERT INTO 
                ORS_tblProgressSummary
                (INVOICE_ID,PAY_INVOICE_ID,PAY_AMOUNT,PAY_CHECK_DATE,PAY_CHECK_NUMBER,PAY_STATUS_LOOKUP_CODE)
            SELECT 
                INVOICE_ID,INVOICE_ID,AMOUNT,CHECK_DATE,CHECK_NUMBER,STATUS_LOOKUP_CODE 
            FROM 
                ORS_tblProgressPay
        ";
        return $this->getArrRes($this->execQry($sql));
    ...
    # IMPORT LINE TO SUMMARY
    
    function importLineDetail(){
        $sql = "    
            INSERT INTO 
                ORS_tblProgressSummary
                (INVOICE_ID,LINE_INVOICE_ID,LINE_GL_LINE_CODE,LINE_AMT_SUM,LINE_TAX_CLASSIFICATION_CODE,LINE_NAME,LINE_DESCRIPTION)
            SELECT 
                A.INVOICE_ID,
                A.INVOICE_ID, 
                A.GL_LINE_CODE, 
    ...
    function exportSummary(){
        $sql = "    
            select 
                INVOICE_ID,SEGMENT1,VENDOR_NAME,ORG_ID,VENDOR_SITE_CODE,INVOICE_NUM,INVOICE_DATE,INVOICE_AMOUNT,SOURCE,
                DESCRIPTION,AMOUNT_REMAINING,LINE_GL_LINE_CODE,LINE_AMT_SUM,LINE_TAX_CLASSIFICATION_CODE,
                LINE_NAME,LINE_DESCRIPTION,PAY_AMOUNT,PAY_CHECK_DATE,PAY_CHECK_NUMBER,PAY_STATUS_LOOKUP_CODE
            from 
                ORS_tblProgressSummary
            ORDER BY 
                INVOICE_ID
