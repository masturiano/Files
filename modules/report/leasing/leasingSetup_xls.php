    ...
    $worksheet->write(2,17,"TERM COMMENTS",$headerFormat);
		...
		foreach ($arrLeasingSetup as $valD) {
            ...
            if(($valD['LEAS_ID'] != '' && is_numeric($valD['LEAS_ID'])) && $valD['PAY_PUR_CODE'] != '' && $valD['PAY_PUR_CODE'] == 'BASE RENT'){
                ...
                //if(stripos($valD['TRX_NUMBER'],"WATUB") || stripos($valD['TRX_NUMBER'],"EATUB")){
                    $rowStoreInfo3 = $leasingSetupObj->findCommentsAtub($valD['TRX_NUMBER']);
                    $worksheet->write($ctr,13,'',$row1);
                    $worksheet->write($ctr,14,'',$row1);
                    $worksheet->write($ctr,15,'',$row1);
                    $worksheet->write($ctr,16,'',$row1);
                    $dateFrom = (date('m/d/Y',strtotime($rowStoreInfo3['mmrbs_period_from']))=='01/01/1970') ? '':date('m/d/Y',strtotime($rowStoreInfo3['mmrbs_period_from']));
                    $dateTo = (date('m/d/Y',strtotime($rowStoreInfo3['mmrbs_period_to']))=='01/01/1970') ? '':date('m/d/Y',strtotime($rowStoreInfo3['mmrbs_period_to']));
                    $worksheet->write($ctr,17,$dateFrom.' - '.$dateTo,$row1);
                /*
                }else{
                    $worksheet->write($ctr,13,'',$row1);
                    $worksheet->write($ctr,14,'',$row1);
