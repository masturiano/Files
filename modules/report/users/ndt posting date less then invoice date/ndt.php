...
switch($_GET['action']){
	...
	case "GETAUTO":
			$arrResult = array();
				$arr= $ndtObj->findCustomer($_GET['term']);
				foreach($arr as $val){
					$arrResult[] = array(
						"id"=>$val['cusnum'],
	...
	case 'Print':
			echo "window.open('ndt_xls.php?{$_SERVER['QUERY_STRING']}');";
	exit();
...
<html>
	<head>
		...
		$(function(){
            var dates = $( "#txtDateFrom" ).datepicker({
                defaultDate: "+0w",
                changeMonth: true,
                numberOfMonths: 1,
		...
		$(function() {
			$( "#tags" ).autocomplete({
				source: function(request, response) {
					$.getJSON('ndt.php?action=GETAUTO', {
						term: request.term
					}, response);
				},
            ...
            <form name="formInq" id="formInq">
                <div class="dvContainer">
                	<table border=0 align="center">
						<th colspan="6">
                        	<h4 align="center"><font style="font-family:Lucida Handwriting"> RCR NDT vs Oracle</font></h4>
                        </th>
						<tr>
							<td colspan = "6"> </td>
						</tr>
                        <tr>
                            ...
                            <td><? $ndtObj->DropDownMenu($arrOrgId,'cmbOrgId','','class="selectBox ui-widget-content ui-corner-all"'); ?></td>
                        </tr>
                        <tr>
                            <td colspan = "6"> </td>
                        </tr>
						<tr>
							...
							<td><font style="font-size:13px"> Date </font></td>
							<td><font style="font-size:13px"> : </font></td>
