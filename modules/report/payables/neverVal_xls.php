...
################### INCLUDE FILE #################
	...
	$filename = "Never_Validated_".$_GET['cmbSource']."_".$_GET['cmbOrgId'].".xls";
	$workbook->send($filename);
	$worksheet = &$workbook->addWorksheet("Cancelled STS");
	$worksheet->setLandscape();
	$worksheet->freezePanes(array(3,0));
	
	$worksheet->write(0,0,"Never Validated From ".date('m/d/Y',strtotime($_GET['txtDateFrom']))." To ".date('m/d/Y',strtotime($_GET['txtDateTo'])),$headerFormat);
	...
	$worksheet->write(2,9,"VENDOR SITE CODE",$headerFormat);
		...
		$arrDailyPaidProc = $neverValObj->neverValDidNotPasXls($_GET['txtDateFrom'],$_GET['txtDateTo'],$_GET['cmbOrgId'],$_GET['cmbSource']);
