...
//$userAcess = $needsRevalObj->login($userName,$passWord);
//$userNotExist = $needsRevalObj->getRecCount($needsRevalObj->loginNotExist($userName,$passWord));
...
switch($_GET['action']){
	...
	case 'Print':
		...
		echo "window.open('needsReval_xls.php?{$_SERVER['QUERY_STRING']}');";	
		//$needsRevalObj->dailyPaidProcessCsv($GET['txtDateFrom'],$GET['txtDateTo'],$GET['cmbOrgId']);
		...
		$(function() {
			$( "#tags" ).autocomplete({
				source: function(request, response) {
					$.getJSON('needsReval.php?action=GETAUTO', {
						term: request.term
					}, response);
				},
            ...
            <form name="formInq" id="formInq">
                <div class="dvContainer">
                	<table border=0 align="center">
						<th colspan="6">
                        	<h4 align="center"><font style="font-family:Lucida Handwriting"> Needs Revalidation </font></h4>
                        </th>
						<tr>
							<td colspan = "6"> </td>
						</tr>
						...
						<tr>
							...
							<td><font style="font-size:13px"> : </font></td>
							<td><? $needsRevalObj->DropDownMenu($arrOrgId,'cmbOrgId','','class="selectBox ui-widget-content ui-corner-all"'); ?></td>
						</tr>
						<tr>
							<td colspan = "6"> </td>
						</tr>
