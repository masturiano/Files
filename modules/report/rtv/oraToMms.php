...
//Connection of copy files from local to 192.168.200.100 
    ...
    "PathNe" =>"/pgtflr/ORACHK/ANEFLR/Unprocess/");
	...
	//ftp_chdir($conn1, $server1["Path"]);  #################################### comment sept 07 2015
...
$arrResetRtvBatchNo = $oraToMmsObj->arrResetRtvBatchNo();

//Reset RTV Batch number
if($arrResetRtvBatchNo['lastRtvBatch'] >= '999999'){
	$arrResetRtvBatchNoExec = $oraToMmsObj->arrResetRtvBatchNoExec();
}
...
switch($_POST['action']){
	...
	case "create":
        ...
        if($library == '1'){
            ...
            ftp_chdir($conn1, $server1["PathNe"]);    
        }
        else{
            echo "$().toastmessage('showErrorToast','Error destination of file!');";
        }
		...
		$rs = $db->Execute("
			SELECT 
			DENSE_RANK() 
      		OVER (ORDER BY a.CHECK_NUMBER) AS DRANK,
			a.CHECK_ID,
			a.CHECK_DATE,
			a.CHECK_NUMBER,
			a.CHECKRUN_NAME,
			...
			AND (c.SOURCE = 'RTV'
			AND a.org_id in (85,87,133,113,153)
			...
			if($strlength>183){
				...
				//while(!$rs.EOF){
					...
					$xcontentx .= trim($rs->fields['DRANK']).",";	#RTABCH#AP Batch Number 7
...
$arr_company = array('0'=>"Select Library",'1'=>"Puregold",'2'=>"NE");
?>
...
<html>
	<head>
		...
		function CreateTextfile(){
            ...
            if($('#cmbLib').val()==0 || $('#cmbLib').val()==''){
                    $().toastmessage('showToast', {
                    text: 'Please select library!',
                    sticky: true,
                    position: 'middle-center',
                    type: 'error',
                    closeText: '',
			...
			$.ajax({
				url: 'oraToMms.php',
				type: 'POST',
				data: 'action=create&batchNumber='+$("#getBatch").val()+'&bankAcctNum='+$("#bankAccountNum").val()+'&checkFrom='+$("#checkFrom").val()+'&checkTo='+$("#checkTo").val()+'&library='+$('#cmbLib').val(),
				beforeSend: function() {
                	...
                	<table border=0>
                        ...
                        <tr>
                            ...
                            <td colspan=2>
                                <div class="input-prepend">
                                <span class="add-on"><i class="icon-info-sign "></i></span>
                                <? $oraToMmsObj->DropDownMenu($arr_company,'cmbLib','','class="selectBox"'); ?>
                            </td>
                        </tr>
                        <tr>
                        	<td colspan="3" align="center" style="height:6px;">
