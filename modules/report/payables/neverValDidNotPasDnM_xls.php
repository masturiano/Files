...
################### INCLUDE FILE #################
	...
	$filename = "Never_Validated_Did_Not_Pass_Debit_And_Manual.xls";
	$workbook->send($filename);
	$worksheet = &$workbook->addWorksheet("NVDNP Debit And Manual");
	$worksheet->setLandscape();
	$worksheet->freezePanes(array(3,0));
	
	$worksheet->write(0,0,"Never Validated Did Not Pass Debit And Manual From ".date('m/d/Y',strtotime($_GET['txtDateFrom']))." to Present",$headerFormat);
	...
	$worksheet->write(2,9,"LOOK UP CODE",$headerFormat);
	$worksheet->write(2,10,"VENDOR SITE CODE",$headerFormat);
		...
		$arrDailyPaidProc = $neverValDidNotPasDnMObj->neverValDidNotPasDnMXls($_GET['txtDateFrom'],$_GET['cmbOrgId']);
		foreach ($arrDailyPaidProc as $valD) {
			...
			$worksheet->write($ctr,9,$valD['Invoice_type_lookup_code'],$row2);
			$worksheet->write($ctr,10,$valD['Vendor_Site_Code'],$row2);
